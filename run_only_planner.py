...
def get_LLM_plan(experiment_type, agent, scene_graph, prompt, details, index, log_file_path, temperature):
    if experiment_type == "multi_step":
        return run_multi_step(agent, scene_graph, prompt, log_file_path, temperature)

    if experiment_type == "obj_displaced":
        return run_obj_displaced(details, index, agent, scene_graph, prompt, log_file_path, temperature)

    if experiment_type == "obj_introduced":
        return run_obj_introduced(details, agent, scene_graph, prompt, log_file_path, temperature)

# Handle multi-step experiment type
def run_multi_step(agent, scene_graph, prompt, log_file_path, temperature):

    detect_objects, tagging_module = setup_detector("multi_step", scene_graph)
    action_sequence = agent.planner(args.embodiment, planner_input(scene_graph), prompt, temperature)
...
def run_obj_displaced(details, index, agent, scene_graph, prompt, log_file_path, temperature):
...
def run_obj_introduced(details, agent, scene_graph, prompt, log_file_path, temperature):
...
def main(args: argparse.Namespace):
    ...
    for task, details in task_data.items():
        ...
        for temperature in args.temperature_list:
        
            log_folder = os.path.join(dataset_root, f"tasks/{args.scene}/{args.experiment_type}/{temperature}")
            os.makedirs(log_folder, exist_ok=True)
            log_file_path = os.path.join(log_folder, f'{task}.txt')
            with open('your_file.txt', 'w') as file:
                pass  # No need to write anything; this will clear the file
            print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
            ...
            for index, prompt in enumerate(details['prompts']):
                ...
                action_images_path = dataset_root / f"action_images/{args.scene}/{args.experiment_type}/{temperature}/{task}/{prompt}"
                llm_plan, object, update_sg, detect_objects, tagging_module = get_LLM_plan(experiment_type, agent, scene_graph, prompt, details, index, log_file_path, temperature)
                ...
                while llm_plan[step] != "standby":
                    ...
                    if visual_feedback is not None:
                        ...
                        replanned_sequence = replanning_agent.replanner(
                            ...
                            action_history, visual_feedback, temperature
                        )
                    ...
                    if action_feedback is not None:
                        ...
                        replanned_sequence = replanning_agent.replanner(
                            args.embodiment, planner_input(scene_graph), prompt, 
                            action_history, action_feedback, temperature
                        )
